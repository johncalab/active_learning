...
class ConllDataset(Dataset):
    def __init__(self,data,char_vzr,word_vzr,tag_vzr):
        self.data = data
        self.char_vzr = char_vzr
        self.word_vzr = word_vzr
        self.tag_vzr = tag_vzr
    ...
    def __getitem__(self,i: int) -> dict:
        ...
        tag_seq = sample["ner"]
        tag_seq = self.tag_vzr.vectorize_sequence(tag_seq)

        seq = sample['words']
        word_seq = self.word_vzr.vectorize_sequence(seq)
        char_seq = self.char_vzr.vectorize_batch([seq])[0]

        return {"char":char_seq, "word":word_seq,"tag":tag_seq}
